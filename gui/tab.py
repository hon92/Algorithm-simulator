...
class ProjectTab(Tab):
    ...
    def run_visual_simulation(self,
        ...
        graph = self.project.graph_manager.get_visible_graph(filename)
...
#     def _run_vizual_simulations(self):
#         for row in self.liststore:
#             if row[0]:
...
class SimulationDetailTab(CloseTab):
    ...
    def create_plots(self):
        ...
        memory_usage_plot = plot.MemoryUsagePlot(self.simulation)
        calculated_plot = plot.CalculatedBarPlot(self.simulation)
        processes_plot = plot.ProcessesLifePlot(self.simulation.ctx.processes)
    ...
    def on_page_switch(self, notebook, page, num):
        ...
        if current_page_index != -1:
            tab = self.notebook.get_nth_page(current_page_index)
...
class VisualSimulationTab(CloseTab):
    ...
    CANVAS_BACKGROUND_COLOR = (240,240,240)
    ...
    def post_build(self):
        ...
        self.sim_stats = statistics.SimulationStatistics(self.info_store,
                                                         self.simulation)
        #self.sim_stats.init_properties()
        #self.controller.run()
...
class SimulationsTab(Tab):
    ...
    def __init__(self, window, project):
        ...
        self.worker = worker.SimWorker(lambda s: gtk.idle_add(self.on_simulation_complete, s),
                                       lambda e: gtk.idle_add(self.on_simulation_error, e))

        self.worker.connect("task_start", lambda s: gtk.idle_add(self.on_simulation_start,
                                                                 s,
    ...
    def _update_title(self):
        self.set_title("Simulations {0} / {1} ".format(self.sim_actual + 1, self.sim_count))
    ...
    def on_timeout(self):
        ...
        self.liststore[self.sim_actual][3] =  int(new_val * 100)
        self.liststore[self.sim_actual][4] = self.RUNNING
        return True
    ...
    def on_simulation_start(self, simulation):
        ...
        for pr in simulation.ctx.processes:
            pr.connect("log", lambda m, t: gtk.idle_add(self.win.console.writeln,
                                                        m,
                                                        t,
                                                        priority = gobject.PRIORITY_HIGH))
    ...
    def on_simulation_complete(self, simulation):
        ...
        self.completed_simulations[self.sim_actual] = simulation
        self.liststore[self.sim_actual][3] = 100
        self.liststore[self.sim_actual][4] = self.COMPLETED
        self.liststore[self.sim_actual][5] = simulation.ctx.env.now
        self.liststore[self.sim_actual][6] = memory_peak
    ...
    def on_simulation_error(self, error_message):
        ...
        self.liststore[self.sim_actual][3] = 0
        self.liststore[self.sim_actual][4] = status_text
        self.liststore[self.sim_actual][5] = 0
        self.liststore[self.sim_actual][6] = 0
